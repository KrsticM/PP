...
global_var
  : type _ID _SEMICOLON
  {
  	if(lookup_symbol($2, GLVAR) == -1)
	{
    	insert_symbol($2, GLVAR, $1, NO_ATR, NO_ATR);
		code("\n\t\t\t%s:	WORD	1", $2);	
	}	
    else 
...
variable
  : type _ID _SEMICOLON
      {
        if(lookup_symbol($2, VAR|PAR|GLVAR) == -1)
...
inc_statement
  : _ID _INC _SEMICOLON
	  {
		int idx = lookup_symbol($1, (VAR|PAR|GLVAR));
        if(idx == -1)
          err("'%s' undeclared", $1);
		code("\n\t\t\tADDS\t");
