...
for_statement
  :	_FOR type _ID
	{
			int idx = insert_symbol($3, VAR, $2, ++var_num, ++for_num);
			$<i>$ = idx;
	}
  _ASSIGN literal
	{
 	...
 	_DIRECTION literal
	{
			...
			{
				if($8 == 0)
						code("\nJGES\t@for_exit%d", get_atr2($<i>4));
				else
						code("\nJLES\t@for_exit%d", get_atr2($<i>4));
			}
			else
			{
				if($8 == 0)
						code("\nJGEU\t@for_exit%d", get_atr2($<i>4));
				else
						code("\nJLEU\t@for_exit%d", get_atr2($<i>4));
			}
	} 
	steping statement
	{
			if($8 == 0)
			{
					code("\n\t\t\t%s\t", get_arop_stmt(ADD, get_type($<i>4)));
			}
			else
			{
					code("\n\t\t\t%s\t", get_arop_stmt(SUB, get_type($<i>4)));
			}
			print_symbol($<i>4);
			code(",");
			if($11 == -1)
			...
			print_symbol($<i>4);
			code("\n\t\t\tJMP \t@for%d", get_atr2($<i>4));
			code("\n@for_exit%d:", get_atr2($<i>4));
			clear_symbols($<i>4);
	}
  ;
