...
argument
  ...
  ;
//for <for_variable> "=" <init_constant> "downto" <final_constant> "do" <for_body>
for_statement
  : _FOR _ID _ASSIGN literal _DOWNTO literal _DO statement
  {
    int idx =  lookup_symbol($2, (VAR|PAR));
    if(idx == -1)
      err("'%s' undeclared", $2);
    if(get_type(idx) != get_type($4) || get_type(idx) != get_type($6))
