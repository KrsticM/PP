...
*/
compare_statement
  : _COMPARE _LPAREN _ID _SEMICOLON _ID _RPAREN _GREATER statement _LESS statement _EQUAL statement
  {
    int idxOne = lookup_symbol($3, (VAR|PAR));
    if(idxOne == -1)
       err("'%s' undeclared", $3);
    int idxSecond = lookup_symbol($5, (VAR|PAR));
    if(idxSecond == -1)
       err("'%s' undeclared", $5);
