...
// Vraca indeks pronadjenog simbola na nivou ili vraca -1.
int level_lookup_symbol(char *name, unsigned kind, unsigned variable_level) 
{
  int i;
  for(i = first_empty - 1; i > FUN_REG; i--) 
  {
    if(strcmp(symbol_table[i].name, name) == 0 && symbol_table[i].kind & kind && symbol_table[i].atr2 == variable_level)
    {
       return i;
    }
  }
...
int level_first_sym(unsigned level)
{
  int i;
  int first = -1;
  for( i = first_empty - 1; i > FUN_REG; i-- )  
  {
    if( symbol_table[i].atr2 == level ) 
    {
      first = i;
    }
  }
...
// Brise elemente sa jednog nivoa
void level_clear_symbols(unsigned level)
{
   int begin_index = level_first_sym( level );
   clear_symbols( begin_index );
}
